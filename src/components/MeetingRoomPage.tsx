...
export default function MeetingRoomsPage({
  ...
  >(undefined);
  const [meetingRooms, setMeetingRooms] = useState<MeetingRoom[] | undefined>(
    undefined,
  );
  ...
  useEffect(() => {
    const fetchCoworkingSpace = async () => {
      ...
      const resMr = await getMeetingRooms(
        undefined,
        coworkingSpaceId,
        reserveDateStart,
        reserveDateEnd,
  ...
  if (!coworkingSpace)
    return <p>No coworking space with id {coworkingSpaceId}.</p>;
